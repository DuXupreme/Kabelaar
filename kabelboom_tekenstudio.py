...
def parse_step_geometry(step_path: Path) -> StepGeometry3D:
    ...
    for pid, xs, ys, zs in re.findall(
        ...
        points_by_id[int(pid)] = (float(xs), float(ys), float(zs))
    ...
    for v1, v2 in re.findall(
        r"#\d+\s*=\s*EDGE_CURVE\s*\(\s*[^,]*,\s*#(\d+)\s*,\s*#(\d+)\s*,",
        text,
        flags=re.IGNORECASE,
    ):
        vv1 = int(v1)
        vv2 = int(v2)
        p1_id = vertex_to_point.get(vv1)
        p2_id = vertex_to_point.get(vv2)
        if p1_id in points_by_id and p2_id in points_by_id:
...
def preview_project(point: Tuple[float, float, float], yaw: float, pitch: float) -> Tuple[float, float]:
    ...
    text = step_path.read_text(encoding="utf-8", errors="ignore")
    matches = re.findall(
        r"CARTESIAN_POINT\s*\(\s*'[^']*'\s*,\s*\(\s*([+-]?\d+(?:\.\d+)?(?:[Ee][+-]?\d+)?)\s*,\s*([+-]?\d+(?:\.\d+)?(?:[Ee][+-]?\d+)?)\s*,\s*([+-]?\d+(?:\.\d+)?(?:[Ee][+-]?\d+)?)",
        text,
        flags=re.IGNORECASE,
    )
    points = [project_xyz(float(x), float(y), float(z), projection) for x, y, z in matches]
    if len(points) < 6:
        points = [(0.0, 0.0), (20.0, 0.0), (20.0, 10.0), (0.0, 10.0), (0.0, 0.0)]
        polylines = [points]
    else:
        step = max(1, len(points) // 6000)
        sampled = points[::step]
        outline = envelope_outline(sampled)
        polylines = [outline]

    normalized, w, h = normalize_polylines(polylines)
    return normalized, w, h
...
class HarnessDrawingStudio(tk.Tk):
    def __init__(self):
        super().__init__()
    ...
    def _build_ui(self):
        ...
        blocks_frame.grid(row=1, column=0, sticky="ew", pady=(8, 0))
